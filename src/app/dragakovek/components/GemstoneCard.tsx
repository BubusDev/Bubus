...
function RelatedThumbs({ gemstones, currentId }: { gemstones: Gemstone[]; currentId: string }) {
  ...
  if (related.length === 0) {
    return <p className="text-sm text-[#7a2a3e]">Hamarosan erkeznek parositasok.</p>;
  }
...
function GemstoneCard({
  ...
  return (
    ...
    >
      <div className="grid min-h-[72vh] border-b border-[#fdfaf7]/10 md:grid-cols-[0.75fr_3fr_0.45fr]">
        <div className="flex items-start border-[#fdfaf7]/10 px-4 py-8 md:border-r md:px-8 md:py-12">
          <span className="font-serif text-7xl font-light leading-none text-[#fdfaf7]/70 sm:text-8xl">
            {index + 1}
          </span>
        </div>

        <div className="grid gap-8 border-[#fdfaf7]/10 px-4 pb-8 md:grid-cols-[minmax(220px,400px)_1fr] md:border-r md:px-8 md:py-12 lg:gap-12">
          <div className="relative aspect-square w-full max-w-[400px] overflow-hidden bg-[#fdfaf7]/15">
            {gemstone.imageUrl ? (
              <Image
                ...
                className="object-cover saturate-[0.68] contrast-[0.92]"
              />
            ) : (
          ...
          <div className="flex min-w-0 flex-col justify-center">
            ...
            <AnimatePresence initial={false}>
              {open && (
                ...
                >
                  <div className="mt-8 space-y-7 border-t border-[#fdfaf7]/10 pt-8">
