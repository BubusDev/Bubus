...
type OrderConfirmationLocale = "hu" | "en";
...
function getOrderLinks(accessModel: OrderAccessModel, locale: OrderConfirmationLocale) {
  ...
  if (accessModel === "authenticated") {
    return {
      ...
      secondary: {
        ...
        href: new URL(prefix || "/", baseUrl).toString(),
      },
    };
  }
...
function getLocalizedOrderLinks(accessModel: OrderAccessModel, locale: OrderConfirmationLocale) {
  const links = getOrderLinks(accessModel, locale);
  if (locale === "hu") return links;
  return accessModel === "authenticated"
    ? {
        primary: { ...links.primary, label: "Open my orders" },
        secondary: { ...links.secondary, label: "Continue shopping" },
      }
    : {
        primary: { ...links.primary, label: "Open order status" },
        secondary: { ...links.secondary, label: "Contact" },
      };
}
...
function renderItemsTableEn(items: OrderConfirmationItem[]) {
  return items
    .map(
      (item) => `
        <tr>
          <td style="padding: 12px 0; border-bottom: 1px solid #ece7e2; color: #1f1a17; font-size: 14px;">
            <strong>${escapeHtml(item.name)}</strong><br />
            <span style="color: #7a7068;">${item.quantity} × ${escapeHtml(item.unitPriceLabel)}</span>
          </td>
          <td style="padding: 12px 0; border-bottom: 1px solid #ece7e2; color: #1f1a17; font-size: 14px; text-align: right; white-space: nowrap;">
            ${escapeHtml(item.lineTotalLabel)}
          </td>
...
function renderHuTemplate(input: OrderConfirmationEmailInput): EmailTemplate {
  const links = getLocalizedOrderLinks(input.accessModel, "hu");
...
function renderEnTemplate(input: OrderConfirmationEmailInput): EmailTemplate {
  const links = getLocalizedOrderLinks(input.accessModel, "en");
  const escapedOrderNumber = escapeHtml(input.orderNumber);
  const escapedCreatedAt = escapeHtml(input.createdAtLabel);
  const escapedTotal = escapeHtml(input.totalLabel);
  const escapedShippingName = escapeHtml(input.shippingName);
  ...
  const intro =
    input.accessModel === "authenticated"
      ? "Thank you for your order. Your payment was successful and we have recorded your order."
      : "Thank you for your order. Your payment was successful and we have recorded your order. Keep this email for your order details.";
  const nextStepNote =
    input.accessModel === "authenticated"
      ? "You can also find the full order history in your account."
      : "You can follow your order on the order status page in this browser. If you have questions, contact us anytime.";
    ...
    <div style="margin: 0; padding: 32px 16px; background: #f6f2ed; font-family: Georgia, 'Times New Roman', serif; color: #1f1a17;">
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width: 680px; margin: 0 auto; background: #ffffff; border: 1px solid #ece7e2;">
        <tr>
          <td style="padding: 32px 32px 20px; border-bottom: 1px solid #ece7e2;">
            <div style="font-size: 11px; letter-spacing: 0.32em; text-transform: uppercase; color: #9a7b63; margin-bottom: 12px;">${BRAND_NAME}</div>
            <h1 style="margin: 0 0 10px; font-size: 28px; line-height: 1.2; font-weight: 600;">Order confirmation</h1>
            <p style="margin: 0; font-size: 15px; line-height: 1.7; color: #5c524b;">${escapeHtml(intro)}</p>
          </td>
        </tr>
        <tr>
          <td style="padding: 28px 32px;">
            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin-bottom: 28px;">
              <tr>
                <td style="width: 50%; padding-right: 12px; vertical-align: top;">
                  <div style="font-size: 11px; letter-spacing: 0.16em; text-transform: uppercase; color: #8c8178; margin-bottom: 6px;">Order number</div>
                  <div style="font-size: 15px; line-height: 1.7;"><strong>${escapedOrderNumber}</strong><br /><span style="color: #5c524b;">${escapedCreatedAt}</span></div>
                </td>
                <td style="width: 50%; padding-left: 12px; vertical-align: top; text-align: right;">
                  <div style="font-size: 11px; letter-spacing: 0.16em; text-transform: uppercase; color: #8c8178; margin-bottom: 6px;">Total</div>
                  <div style="font-size: 22px; line-height: 1.2; font-weight: 600;">${escapedTotal}</div>
                </td>
              </tr>
            </table>
            <div style="margin-bottom: 28px;">
              <div style="font-size: 11px; letter-spacing: 0.16em; text-transform: uppercase; color: #8c8178; margin-bottom: 10px;">Order summary</div>
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0">${renderItemsTableEn(input.items)}</table>
            </div>
            <div style="margin-bottom: 28px; padding: 18px 20px; background: #faf7f3; border: 1px solid #ece7e2;">
              <div style="font-size: 11px; letter-spacing: 0.16em; text-transform: uppercase; color: #8c8178; margin-bottom: 8px;">Shipping address</div>
              <div style="font-size: 14px; line-height: 1.7;"><strong>${escapedShippingName}</strong><br />${escapedShippingAddress}</div>
            </div>
            <div style="margin-bottom: 26px; font-size: 14px; line-height: 1.7; color: #5c524b;">${escapeHtml(nextStepNote)}</div>
            <table role="presentation" cellspacing="0" cellpadding="0"><tr><td style="padding-right: 12px;"><a href="${escapeHtml(links.primary.href)}" style="display: inline-block; background: #1f1a17; color: #ffffff; text-decoration: none; padding: 13px 18px; font-size: 14px;">${escapeHtml(links.primary.label)}</a></td><td><a href="${escapeHtml(links.secondary.href)}" style="display: inline-block; background: #ffffff; color: #1f1a17; text-decoration: none; padding: 13px 18px; font-size: 14px; border: 1px solid #d9d0c8;">${escapeHtml(links.secondary.label)}</a></td></tr></table>
          </td>
        </tr>
      </table>
    </div>
